...
// (the app itself returns to the welcome screen on reload), and falls back to
// an in-memory id if storage is unavailable (private mode, etc.).
...
interface DeviceInfo {
  ...
  os: string
  browser: string
  /** Whether the device can share an image via the native share sheet. */
...
export function getDeviceInfo(): DeviceInfo {
  ...
  try {
    if (typeof navigator.share === 'function') {
      ...
      canShare = nav.canShare?.({ files: [probe] }) ?? false
    }
  } catch {
    /* ignore */
  }
